...
// Joins given list of automata into single DFA with accepting states marked
// with Result corresponding to source DFAs.
// Throws if any state would be marked by many results, i.e. when any two DFAs accept same word.
fun <State, Atom, Result> joinAutomata(
    ...
    // Get result for given state or null if it is not accepting.
    // Throws if result is ambiguous.
    ...
    while (!queue.isEmpty()) {
        ...
        // Throws if currentState has ambiguous Result
