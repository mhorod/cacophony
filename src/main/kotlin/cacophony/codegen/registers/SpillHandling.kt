...
fun adjustLoweredCFGToHandleSpills(
    ...
    val spillsFrameAllocation = allocateFrameMemoryForSpills(callableHandler, spillsColoring)
...
private fun allocateFrameMemoryForSpills(
    ...
    val colorToFrameMemory =
        mutableMapOf<Int, CFGNode.LValue>().apply {
            spillsColoring.forEach { (reg, color) ->
                if (get(color) == null) {
                    put(color, callableHandler.allocateFrameVariable(Variable.PrimitiveVariable(reg.holdsReference)))
                }
            }
        }
