...
class ExamplesTest {
    ...
    private fun runTest(input: Input): TestResult {
        ...
        try {
            ...
            result.resolvedVariables = pipeline.checkTypes(result.ast!!, nameResolutionResult).resolvedVariables
            val variablesMap = pipeline.createVariables(result.ast!!, result.types!!)
            result.analysisResult = pipeline.analyzeFunctions(result.ast!!, variablesMap, result.resolvedVariables!!)
